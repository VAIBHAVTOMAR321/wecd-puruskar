...
const successData = [
  {
    ...
    icon: <FaSchool />,
    theme: { bg: "#eaf8fa", border: "#b6ecf2", iconBg: "#cdf2f7", iconColor: "#0891b2", numColor: "#056380", barColor: "#06b6d4", barTrack: "#b6ecf2", shadow: "rgba(6,182,212,0.08)", hoverShadow: "rgba(6,182,212,0.18)", accent: "#06b6d4" },
  },
  {
    village: "हरिद्वार",
    registered: 22000,
    ...
    icon: <FaHeartbeat />,
    theme: { bg: "#fef0ef", border: "#fcd5d2", iconBg: "#fde2e0", iconColor: "#dc2626", numColor: "#a01a1a", barColor: "#ef4444", barTrack: "#fcd5d2", shadow: "rgba(220,38,38,0.08)", hoverShadow: "rgba(220,38,38,0.18)", accent: "#ef4444" },
  },
  {
    village: "नैनीताल",
    registered: 18000,
    ...
    icon: <FaLeaf />,
    theme: { bg: "#edfaf4", border: "#b8ecd0", iconBg: "#d0f5e2", iconColor: "#0d8043", numColor: "#06612f", barColor: "#10b981", barTrack: "#b8ecd0", shadow: "rgba(13,128,67,0.08)", hoverShadow: "rgba(13,128,67,0.18)", accent: "#10b981" },
  },
  {
    ...
    icon: <FaHardHat />,
    theme: { bg: "#fef9ee", border: "#fce3b3", iconBg: "#fdf0d0", iconColor: "#b8860b", numColor: "#8b6508", barColor: "#d4a017", barTrack: "#fce3C3", shadow: "rgba(184,134,11,0.08)", hoverShadow: "rgba(184,134,11,0.18)", accent: "#d4a017" },
  },
  {
    village: "अल्मोड़ा",
    registered: 15500,
    ...
    icon: <FaUniversity />,
    theme: { bg: "#f5effe", border: "#e0ccff", iconBg: "#ecddff", iconColor: "#7c3aed", numColor: "#5b21b6", barColor: "#8b5cf6", barTrack: "#e0ccff", shadow: "rgba(124,58,237,0.08)", hoverShadow: "rgba(124,58,237,0.18)", accent: "#8b5cf6" },
  },
];
...
const totalRegistered = successData.reduce((s, i) => s + i.registered, 0);
const totalCompleted = successData.reduce((s, i) => s + i.completed, 0);
const overallSuccessRate = totalRegistered > 0 ? Math.round((totalCompleted / totalRegistered) * 100) : 0;
const allDepartments = [...new Set(successData.flatMap((i) => i.departments))];
const allSchemes = [...new Set(successData.flatMap((i) => i.schemes.map((s) => s.name)))];
...
function SuccessStory() {
  ...
  const statCards = [
    { icon: <FaMapMarkerAlt />, value: `${animVillages}+`, label: "Villages Covered", ref: villagesRef, theme: statThemes[0] },
    { icon: <FaUsers />, value: `${animRegistered.toLocaleString("en-IN")}+`, label: "Students Registered", ref: registeredRef, theme: statThemes[1] },
    { icon: <FaCheckCircle />, value: `${animCompleted.toLocaleString("en-IN")}+`, label: "Successful Completions", ref: completedRef, theme: statThemes[2] },
    { icon: <FaPercentage />, value: `${animRate}%`, label: "Overall Success Rate", ref: rateRef, theme: statThemes[3] },
  ];
    ...
    <section className="success-story-section">
      <Container>
        ...
        <div className="ss-section-heading">
          <div className="ss-section-badge"><FaTrophy /><span>Our Achievements</span></div>
          <h2>Impact & Success Stories</h2>
          <p>हमारे प्रयासों से गांवों में हुई सकारात्मक बदलाव की कहानियाँ</p>
        ...
        <div className="ss-filters-card">
          <Row className="g-3 align-items-center">
            <Col lg="auto"><div className="ss-filter-title"><FaFilter /><span>Filters</span></div></Col>
            <Col md><Form.Select name="village" value={filters.village} onChange={handleFilterChange} className="ss-filter-select"><option value="all">All Villages</option>{successData.map((item) => <option key={item.village} value={item.village}>{item.village}</option>)}</Form.Select></Col>
            <Col md><Form.Select name="department" value={filters.department} onChange={handleFilterChange} className="ss-filter-select"><option value="all">All Departments</option>{allDepartments.map((d) => <option key={d} value={d}>{d}</option>)}</Form.Select></Col>
            <Col md><Form.Select name="scheme" value={filters.scheme} onChange={handleFilterChange} className="ss-filter-select"><option value="all">All Schemes</option>{allSchemes.map((s) => <option key={s} value={s}>{s}</option>)}</Form.Select></Col>
          </Row>
        </div>
          ...
          <div className="ss-chart-structure">
            <Row className="g-4">
              <Col lg={7}>
                <div className="ss-chart-main">
                  <div className="ss-chart-main-header">
                    ...
                    <div><h4>Village Performance</h4><p>Registered vs Completed comparison</p></div>
                  </div>
              ...
              <Col lg={5} className="d-none d-lg-block">
                <div className="ss-breakdown-panel">
                  <div className="ss-breakdown-header"><FaBullseye /><h4>Village Breakdown</h4></div>
                  <div className="ss-breakdown-list">
                    {filteredData.map((item, i) => {
                      const rate = item.registered > 0 ? Math.round((item.completed / item.registered) * 100) : 0;
                      const t = item.theme;
                      return (
                        <div key={i} className="ss-breakdown-item" style={{ "--bd-bg": t.bg, "--bd-border": t.border, "--bd-icon-bg": t.iconBg, "--bd-icon-color": t.iconColor, "--bd-num-color": t.numColor, "--bd-bar": t.barColor, "--bd-bar-track": t.barTrack }}>
                          <div className="ss-breakdown-item-header">
                            <div className="ss-breakdown-icon">{item.icon}</div>
                            <div className="ss-breakdown-info"><strong>{item.village}</strong><span>{rate}% success rate</span></div>
                          ...
                          <div className="ss-breakdown-bars">
                            <div className="ss-breakdown-bar-row">
                              <span className="ss-breakdown-bar-label">Registered</span>
                            ...
                            <div className="ss-breakdown-bar-row">
                              <span className="ss-breakdown-bar-label">Completed</span>
            ...
            <div className="ss-comparison-strip">
              {filteredData.map((item, i) => {
                ...
                const t = item.theme;
                return (
                  <div key={i} className="ss-compare-item" style={{ "--cmp-bg": t.bg, "--cmp-border": t.border, "--cmp-icon-color": t.iconColor, "--cmp-bar": t.barColor, "--cmp-bar-track": t.barTrack }}>
                    <div className="ss-compare-icon">{item.icon}</div>
        ...
        <Row className="g-4 ss-stories-row">
          {filteredData.length > 0 ? (
            filteredData.map((item, index) => {
              ...
              return (
                <Col md={6} lg={4} key={index}>
                  <div className="ss-village-card" style={{ "--v-bg": t.bg, "--v-border": t.border, "--v-icon-bg": t.iconBg, "--v-icon-color": t.iconColor, "--v-num-color": t.numColor, "--v-bar": t.barColor, "--v-bar-track": t.barTrack, "--v-shadow": t.shadow, "--v-hover-shadow": t.hoverShadow, "--v-accent": t.accent }}>
                    <div className="ss-village-top-bar" />
                    <div className="ss-village-header">
                      <div className="ss-village-icon">{item.icon}</div>
                      <div className="ss-village-title-wrap"><h4>{item.village}</h4><span className="ss-success-badge">{successRate}% Success</span></div>
                    </div>
                    <div className="ss-village-story"><FaQuoteLeft /><p>{item.story}</p></div>
                    <div className="7ss-village-stats">
                      <div className="ss-village-stat-item"><strong>{item.registered.toLocaleString("en-IN")}</strong><span>Registered</span></div>
                      <div className="ss-village-stat-divider" />
                      <div className="ss-village-stat-item"><strong>{item.completed.toLocaleString("en-IN")}</strong><span>Completed</span></div>
                    </div>
            ...
            <Col>
              <div className="ss-no-results"><FaTrophy /><h4>No matching stories found</h4><p>Please adjust your filters.</p></div>
            </Col>
          )}
        </Row>
      </Container>
